...
const handleSuccessfulPayment = async (client, userId, amount, paymentId, loanId = null) => { // Added client parameter
  try {
    let token = null;
    let tokenExpirationDays = 30; // Default to 30 days if no loan or specific calculation

    if (loanId) {
      const loanResult = await client.query('SELECT payment_amount_per_cycle, device_id, payment_frequency, payment_cycle_amount FROM ray_loans WHERE id = $1', [loanId]); // client.query
      if (loanResult.rows.length > 0) {
        ...
        const deviceResult = await client.query('SELECT d.serial_number, dt.manufacturer FROM ray_devices d LEFT JOIN ray_device_types dt on d.device_type_id = dt.id  WHERE d.id = $1', [deviceId]); // client.query
    ...
    await client.query( // client.query
      'INSERT INTO ray_tokens (user_id, token, payment_id, expires_at) VALUES ($1, $2, $3, $4)',
      [userId, token, paymentId, new Date(Date.now() + tokenExpirationDays * 24 * 60 * 60 * 1000)]
    );

    // Get user's phone number (assuming it's stored in the ray_users table or can be fetched)
    const user = await client.query('SELECT phone_number FROM ray_users WHERE id = $1', [userId]); // client.query
    ...
    const assignedDevices = await client.query( // client.query
      'SELECT assigned_by FROM ray_devices WHERE assigned_to = $1 AND assigned_by IS NOT NULL',
      [userId]
    );
    console.log(`Assigned ray_devices for customer ${userId}:`, assignedDevices.rows[0]);

    if (assignedDevices.rows.length > 0) {
      const agentId = assignedDevices.rows[0].assigned_by;

      const agentResult = await client.query('SELECT commission_rate, super_agent_id, role FROM ray_users WHERE id = $1', [agentId]); // client.query
      ...
      if (agentResult.rows.length > 0) {
        ...
        if (commissionRate === null || commissionRate == 0 ) {
          const generalRate = await client.query("SELECT setting_value FROM ray_settings WHERE setting_key = $1", ['general_agent_commission_rate']); // client.query
        ...
        if (superAgentId) {
          const superAgentResult = await client.query('SELECT commission_rate FROM ray_users WHERE id = $1', [superAgentId]); // client.query

          if (superAgentResult.rows.length > 0) {
            let superAgentCommissionRate = superAgentResult.rows[0].commission_rate;
            if (superAgentCommissionRate === null || superAgentCommissionRate == 0) {
              const generalSuperRate = await client.query("SELECT setting_value FROM ray_settings WHERE setting_key = $1", ['general_super_agent_commission_rate']); // client.query
            ...
            const newCommission = await client.query( // client.query
              'INSERT INTO ray_commissions (agent_id, customer_id, payment_id, amount, commission_percentage) VALUES ($1, $2, $3, $4, $5) RETURNING id',
              [agentId, userId, paymentId, agentCommission, commissionRate]
            );
            await client.query( // client.query
              'INSERT INTO ray_super_agent_commissions (super_agent_id, agent_id, original_commission_id, amount, commission_percentage) VALUES ($1, $2, $3, $4, $5)',
        ...
        console.log('>>>>>>>>>>>>');
        const SuperComRate = await client.query("SELECT setting_value FROM ray_settings WHERE setting_key = $1", ['general_agent_commission_rate']); // client.query
        ...
        await client.query( // client.query
          'INSERT INTO ray_super_agent_commissions (super_agent_id, agent_id, amount, commission_percentage, original_commission_id) VALUES ($1, $2, $3, $4, $5)',
    ...
    if (loanId) {
      ...
      const specificLoan = await client.query('SELECT id, customer_id, total_amount, amount_paid, balance, term_months, next_payment_date, payment_frequency, payment_amount_per_cycle, status FROM ray_loans WHERE id = $1 AND customer_id = $2', [loanId, userId]); // client.query
      ...
      // If no specific loanId, apply to active ray_loans (existing behavior)
      const activeLoans = await client.query( // client.query
        'SELECT id, customer_id, total_amount, amount_paid, balance, term_months, next_payment_date, payment_frequency, payment_amount_per_cycle, status FROM ray_loans WHERE customer_id = $1 AND status = active ORDER BY created_at ASC',
        [userId]
      );
    ...
    for (const loan of loansToUpdate) {
      ...
      await client.query( // client.query
        'UPDATE ray_loans SET amount_paid = $1, balance = $2, status = $3, next_payment_date = $4, updated_at = $6 WHERE id = $5',
