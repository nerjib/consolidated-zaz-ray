  ...
  try {
    const superAgents = await query(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM ray_devices WHERE assigned_by = u.id OR super_agent_id =u.id ) AS "devicesManaged",
        ...
        (SELECT setting_key, setting_value FROM ray_settings WHERE setting_key IN ('general_super_agent_commission_rate')) AS "genCommission",
